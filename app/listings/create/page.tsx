...
export default function CreateListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const body: any = { title, description, type, images: uploadedImageUrls };
      ...
      if (response.ok) {
        ...
        setError(data.details ? data.details.map((d: any) => d.message).join(", ") : data.error || "Failed to create listing");
      }
    } catch {
  ...
  return (
    <PageContainer size="narrow">
      ...
      <div className="card" style={{ padding: "28px 24px" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 700, marginBottom: "24px" }}>Create New Listing</h1>
